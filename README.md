...
- 消费 / 收入记录
- 月度财政规划：输入工资或生活费和储蓄目标，自动计算可支出额度与每日基准
- 今日、本周、本月、今年总结
- 月度 / 年度收支图
- 心情记录和趋势
- 天气查询
...
- 场景化闲聊，支持温柔、简洁、严格、学长、轻松幽默和安静倾听模式
- 删除、修改、最近记录查询
- 可选：截图 / 小票 OCR 记账
...
直接说：`九月生活费3000，计划省1000`。机器人会保存收入与储蓄目标，将 2000 元作为当月可支出预算，并按当月天数计算每日支出基准。记账让某一天首次超过基准时会提醒，同时给出本月剩余额度和后续每日建议额度。

也可以继续设置分类预算，例如：`设置本月餐饮预算800`。发送 `/budget` 或“预算情况”即可查看进度。补记过去日期的消费时，会按消费实际日期和对应月份计算，不会错误算到今天。
