...
直接说：`九月生活费3000，计划省1000`。机器人会把 2000 元设为当月可支出额度，并按当月天数算出每日支出基准。某一天的记账总额首次超过基准时，它会提醒本月余额和后续每日建议额度。

分类预算仍可单独设置，例如：`设置本月餐饮预算800`。发送 `/budget` 或“预算情况”查看收入、储蓄目标、预算使用比例与剩余金额。补记旧账会按消费实际日期归入对应月份。
